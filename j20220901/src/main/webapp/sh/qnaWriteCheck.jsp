...
<body>
	...
	<div class="profile">
     ...
     <!--     상태 테마 칸  -->
			...
			<div style="border: 1px solid #D5D5D5; margin-left: 350px; width: 300px" class="gubun">
				<div > 
					<br> <br> <span class="wait">상태</span> <span
						class="fw-bold me-2"> <c:if
							test="${board.b_success == 'Y' }">
							<span style="color: red;">${board.b_success }</span>
						</c:if> <c:if test="${board.b_success != 'Y' }">
							<span>${board.b_success }</span>
						</c:if>
					</span> <span class="wait">테마</span> <span class="fw-bold me-2">${board.b_theme }</span>
				</div>
	...
	<div class="tag1">
		...
		<div class="btn-group btn-group-sm" role="group" aria-label="..."
			...
			<span class="hash">#${board.l_hash1}</span>&nbsp; &nbsp; &nbsp; <span
				class="hash">#${board.l_hash2}</span>&nbsp; &nbsp; &nbsp; <span
				class="hash">#${board.l_hash3}</span>
		...
		<div class="content">
			<form action="qnaCommentWrite.do" style="width: 2000px">
				<input type="hidden" name="b_num" value="${board.b_num }"> <input
					type="hidden" name="user_id" value="${board.user_id }">
				<textarea name="com_content" rows="" cols=""
					style="width: 1100px; height: 90px; vertical-align: middle; border-color: #dbdbdb; outline-color: #FF3500"></textarea>
				<input type="submit" value="등록"
					style="vertical-align: middle; width: 120px; height: 90px; background-color: #FF3500; border-color: #FF3500; font-weight: bold; color: white; border-radius: 10%;">
			</form>
		</div>
		...
		<div style="margin-left: 350px">
			<br> <br>
			<div class="count" style="font-size: 14px; font-weight: bold;">
				<font color="#FF3500">${fn:length(list)}</font>개의 답변
			</div>
			...
			<c:forEach var="comment" items="${list }" varStatus="inum">
				...
				<div style="margin-top: 25px;">
					<div style="float: left;">
						<img src="<%=context %>/sh_images/user_icon0${inum.count }.png"
							width="40" height="40"
							class="userIconColor-1 rounded-circle me-2  align-center bg-white">
					</div>
					<div class="fw-bold" style="font-size: 15px; font-weight:300; line-height: 1.1">${comment.user_id }

						<!-- 댓글 삭제 -->
						<c:if test="${user_id == comment.user_id }">
							<span><a class="delete"
								href="<%=context %>/qnaCommentDelete.do?com_num=${comment.com_num}&b_num=${b_num}">
									삭제<img src="<%=context%>/sh_images/trash" width="16"
									height="16" style="vertical-align: sub;">
							</a></span>
						</c:if>
					</div>
